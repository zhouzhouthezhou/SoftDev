...
function play() {
	...
	if((!table_ids.includes(move.value)) || board_state[dict[move.value.charAt(0)] * 3 + parseInt(move.value.charAt(1)) - 1] != -1){
		alert('Please Enter a Valid Move')
	}else{
		var target = dict[move.value.charAt(0)] * 3 + parseInt(move.value.charAt(1)) - 1
		if(turn == 1){
			board_state[target] = 1
			document.getElementById(move.value).innerHTML = "X"
			turninfo.innerHTML = "Turn for: <b>O</b>"
			turn = 0
		}else{
			board_state[target] = 0
			document.getElementById(move.value).innerHTML = "O"
			turninfo.innerHTML = "Turn for: <b>X</b>"
			turn = 1
		}

		var validate = [board_state[0] == board_state[4] && board_state[4] == board_state[8] && board_state[0] != -1, board_state[2] == board_state[4] && board_state[4] == board_state[6] && board_state[2] != -1]
		for(i = 0; i < 3; i++){
			validate.push(board_state[i*3] == board_state[(i*3)+1] && board_state[(i*3)+1] == board_state[(i*3)+2] && board_state[i*3] != -1)
			validate.push(board_state[i] == board_state[i+3] && board_state[i+3] == board_state[i+6] && board_state[i] != -1)
		}
